...
const Header = ({ scrollMarkerRef, NAV_ASSET }) => {
    const containerRef = useRef(null);

    useEffect(() => {
        const getHeader = containerRef.current;
        const getScrollMarker = scrollMarkerRef.current;

        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (!entry.isIntersecting && entry.boundingClientRect.top < 0) {
                    getHeader.classList.add("is-scrolled");
                } else {
                    getHeader.classList.remove("is-scrolled");
                };
            });
        }, {
            threshold: 0
        });
        observer.observe(getScrollMarker);
    }, [scrollMarkerRef]);
        ...
        <header className="header-container" ref={containerRef}>
            ...
            <nav className="header-nav">
                <ul className="nav-list">
                    {NAV_ASSET.map((nav) => {
                        return (
                            <li className="header-nav-item" key={nav.id}>
                                <a href={`#section-${nav.id}`} className="nav-name hover-cursor">
                                {nav.name}</a>
                            </li>
                        );
                    })}
